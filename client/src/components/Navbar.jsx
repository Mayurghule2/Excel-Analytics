    ...
    <header className={`w-full px-4 py-2 shadow-2xl ${isLoggedIn
      ...
      <div className="container mx-auto flex justify-between items-center">
        ...
        <nav className={`flex-col md:flex-row md:flex items-center gap-6 absolute md:static bg-[#030d46] md:bg-transparent shadow-md md:shadow-none w-full md:w-auto top-16 left-0 z-50 p-6 md:p-0 transition-all duration-300 ease-in-out ${showMobileMenu ? 'flex' : 'hidden'}`}>
          {!isLoggedIn && (
            <>
              <Link to="/" className="hover:text-blue-600 font-semibold text-white md:text-black lg:text-black">Home</Link>
              <a href="#about-us" className="hover:text-blue-600 font-semibold text-white md:text-black lg:text-black" onClick={(e) => { e.preventDefault(); document.getElementById("about-us").scrollIntoView({ behavior: "smooth" }); }}>About Us</a>
              <a href="#services" className="hover:text-blue-600 font-semibold text-white md:text-black lg:text-black" onClick={(e) => { e.preventDefault(); document.getElementById("services").scrollIntoView({ behavior: "smooth" }); }}>Services</a>
              <Link to="/contact" className="hover:text-blue-600 font-semibold text-white md:text-black lg:text-black">Contact Us</Link>
              <Link to="/sign-in">
